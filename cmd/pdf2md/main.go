...
func printUsage() {
	...
	fmt.Printf(`USAGE:
   pdf2md                          Launch interactive wizard (prompts for file, dest & settings)
   pdf2md convert <input.pdf>      Convert a single PDF file
   pdf2md batch <directory>        Batch convert all PDFs in a directory
   pdf2md help                     Show this help screen
   pdf2md version                  Show version

OPTIONS (for non-interactive CLI flags):
   -o, -output <path>              Destination folder (default: same directory as PDF)
...
EXAMPLES:
   ...
   # Save into a specific destination directory:
   pdf2md convert Cloud_Guide.pdf -o ~/Documents/Notes/
...
func main() {
	...
	case "version", "-v", "--version":
		fmt.Printf("pdf2md v%s (built with Go %s)\n", version, "1.22")
		return
...
func runInteractiveMode() {
	...
	fmt.Printf("%s⏳ Extracting and transforming:%s %s...\n", ui.ColorCyan, ui.ColorReset, filepath.Base(opts.PDFPath))
	...
	if !opts.SplitByChapters {
		...
		fmt.Printf("\n%s%s✅ Success! Single file created:%s %s\n", ui.ColorGreen, ui.ColorBold, ui.ColorReset, targetFile)
		return
	}
	...
	fmt.Printf("   📂 Destination Folder: %s%s/%s\n", ui.ColorCyan, result.TargetDirectory, ui.ColorReset)
...
func runConvert(args []string) {
	...
	if singleFile {
		...
		fmt.Printf("✅ Success! Single file saved to: %s\n", targetFile)
		return
	}
	...
	fmt.Printf("✅ Success! Extracted into folder: %s/\n", result.TargetDirectory)
	fmt.Printf("   📑 TOC Index: %s/README.md\n", result.TargetDirectory)
	fmt.Printf("   📚 Total Chapters: %d\n", len(result.Chapters))
	for _, ch := range result.Chapters {
		fmt.Printf("      - [%02d] %s -> %s\n", ch.Index, ch.Title, ch.Filename)
...
func runBatch(args []string) {
	...
	if len(pdfFiles) == 0 {
		fmt.Printf("%s⚠️  No PDF files found in '%s'%s\n", ui.ColorYellow, inputDir, ui.ColorReset)
		return
	}

	fmt.Printf("🔍 Found %d PDF file(s) in %s\n", len(pdfFiles), inputDir)
	...
	for _, pdf := range pdfFiles {
		...
		fmt.Printf("   %s✓ Created folder:%s %s/ (%d chapters)\n", ui.ColorGreen, ui.ColorReset, filepath.Base(res.TargetDirectory), len(res.Chapters))
		successCount++
	}

	fmt.Printf("\n%s🎉 Batch processing complete! %d/%d PDF documents converted into chapter folders.%s\nBase destination: %s\n",
		ui.ColorGreen, successCount, len(pdfFiles), ui.ColorReset, outputDir)
}

func ColorReset() string {
	return ui.ColorReset
}
